...
fn parse_line(line: &str) -> (&str, (u32, Vec<&str>)) {
    ...
    let weight: u32 = parent_tokens[1].chars()
        .skip(1)
...
fn is_balanced(key: &str, tree: &HashMap<&str, (u32, Vec<&str>)>) -> Result<BalancedNode, u32> {
    ...
    let (children, unbalanced): (Vec<_>, Vec<_>) = children.iter()
    ...
    if weights.keys().len() <= 1 {
        let children_weight: u32 = weights.iter().map(|(weight, occurrences)| weight * occurrences).sum();
